...
The active JDBC manager now overrides its two read queries for `members(user_id, pw, active)` and `roles(user_id, role)`. Spring Security does not require default physical table names when custom queries return the required values in order: username/password/enabled for the account query and username/authority for the authority query. The enabled value is required by `JdbcDaoImpl`, but it may come from a differently named column such as `active` or from a constant `true`; a physical column named `enabled` is not mandatory. **Recall rule:** custom schema names are flexible; the JDBC result shape is the contract. [Detailed security notes](05-spring-boot-rest-security/00-spring-boot-rest-security-employee-starter-code/notes.md#custom-jdbc-schema-and-enterprise-identity-sources--2026-09-24)
...
181. Which columns and order must the custom JDBC user query return?
182. Is a physical database column named `enabled` required, and how can the query handle its absence?
